...
class ClassificationEvaluator:
    ...
    def evaluate_drop(
        ...
        list_experts = np.arange(total_number_of_experts)
        np.random.shuffle(list_experts)
        list_experts = list_experts[:nb_experts_to_use]
        with torch.no_grad():
            labels_all = []
            logits_all = []
            losses_all = []
            ...
            for batch in tqdm(self.dataloader, desc="Evaluation"):
                ...
                domains = batch[3] if self.use_domain else None
                if self.use_labels:
                    loss, logits = model.forward_test_drop_experts(input_ids, attention_mask=masks, domains=domains, labels=labels, list_experts=list_experts)
                    if len(loss.size()) > 0:
                        loss = loss.mean()
                else:
                    (logits,) = model(input_ids, attention_mask=masks, domains=domains)
